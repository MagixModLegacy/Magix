...
G.createTopInterface=function()
	...
	G.updateSpeedButtons=function()
	{
			...
			if (div)
			{
				...
				else if (speed==3) {if (G.getSetting('animations')) {triggerAnim(l('customSpeedButton'),'plop');} l('pauseButton').classList.remove('on');l('playButton').classList.remove('on');l('fastButton').classList.add('on');l('customSpeedButton').classList.add('on');}
			}
	}
