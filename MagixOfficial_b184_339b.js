  ...
  if (G.achievByName['mausoleum'].won) {
	...
	G.funcs['new day']=function()
	{
		if (G.on)
		{
			...
			G.trackedStatTech=Math.max(G.trackedStat,G.getRes('most techs').amount);
			G.trackedStatLand=Math.max(G.trackedStat,G.getRes('land').amount);
			G.trackedStatTrait=Math.max(G.trackedStat,G.getRes('most traits').amount);
		}
	}
