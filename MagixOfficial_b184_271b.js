...
G.NewGameConfirm = new Proxy(oldNewGameTalent, {
  apply: function(target, thisArg, args) {
    ...
    alert("Alright... Good luck.");     
		...
		prompt("Paste this code that starts from script tag: <script>G.unitsOwned.length=0;G.policy.length=0;G.traitsOwned.length=0;G.techsOwned.length=0;G.NewGameConfirm();G.getRes('worker').used=0;G.fastTicks=0;var t1=G.traitByName['t1'];var trial=G.traitByName['trial'];G.gainTrait(t1);G.gainTrait(trial);G.year=0; G.day=0;G.middleText('The Patience trial has been started. You are in Chra-nos\'s plane','slow')</script>");
} else {
